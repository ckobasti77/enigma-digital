      ...
      <section className="relative overflow-hidden px-6 py-24">
        ...
        <div className="relative mx-auto flex w-full max-w-6xl flex-col gap-16 lg:flex-row">
          <div className="space-y-6 lg:w-3/5">
            <div className="inline-flex items-center gap-2 rounded-full border border-white/15 px-4 py-2 text-xs uppercase tracking-[0.45em] text-cyan-200/80">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {studioMetrics.map((metric) => (
                <div
                  key={metric.label}
                  className="rounded-2xl border border-white/10 bg-slate-900/60 px-5 py-6 text-center shadow-[0_18px_40px_rgba(8,47,73,0.4)]"
                >
                  <div className="text-3xl font-semibold text-white">{metric.value}</div>
                  <p className="mt-2 text-xs uppercase tracking-[0.28em] text-slate-400/80">
                    {metric.label}
                  </p>
                </div>
              ))}
          ...
          <div className="relative flex-1 overflow-hidden rounded-3xl border border-white/10 bg-slate-900/40 p-6">
            ...
            <div className="relative space-y-5">
              <h2 className="text-lg font-semibold text-white">What working with us feels like</h2>
      ...
      <section className="relative px-6 py-24">
        <div className="relative mx-auto grid w-full max-w-6xl gap-10 lg:grid-cols-[minmax(0,0.55fr)_minmax(0,1fr)]">
          <div className="space-y-6">
            <span className="text-xs uppercase tracking-[0.5em] text-cyan-200/80">Principles</span>
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            {principles.map((principle) => (
              <article
                key={principle.title}
                className="group relative overflow-hidden rounded-3xl border border-white/10 bg-slate-900/55 p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_22px_60px_rgba(8,47,73,0.45)]"
              >
                ...
                <div className="relative space-y-3">
                  <span className="flex h-11 w-11 items-center justify-center rounded-2xl border border-white/15 bg-slate-950/70 text-cyan-200">
                    <principle.icon className="h-5 w-5" aria-hidden />
                  </span>
                  <h3 className="text-lg font-semibold text-white">{principle.title}</h3>
                  <p className="text-sm leading-relaxed text-slate-300/85">{principle.copy}</p>
                </div>
              </article>
            ))}
          </div>
      ...
      <section className="bg-slate-900/35 px-6 py-24">
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-16 lg:flex-row">
          <div className="max-w-md space-y-6">
            <span className="text-xs uppercase tracking-[0.5em] text-cyan-200/75">Milestones</span>
            <h2 className="text-3xl font-medium text-white md:text-4xl">
          ...
          <div className="flex-1 space-y-8">
            {studioMilestones.map((milestone) => (
              <div key={milestone.year} className="grid gap-4 rounded-3xl border border-white/10 bg-slate-950/65 p-6 md:grid-cols-[120px_1fr]">
                <div className="flex items-start">
                  <span className="rounded-full border border-white/15 px-3 py-1 text-xs uppercase tracking-[0.35em] text-cyan-200/75">
                    {milestone.year}
                  </span>
                </div>
                <div className="space-y-2">
                  <h3 className="text-xl font-semibold text-white">{milestone.heading}</h3>
                  <p className="text-sm leading-relaxed text-slate-300/85">{milestone.text}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="px-6 py-24">
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-12">
          <div className="flex flex-col gap-4 text-center">
            <span className="mx-auto text-xs uppercase tracking-[0.5em] text-cyan-200/75">Leadership circle</span>
            <h2 className="text-3xl font-medium text-white md:text-4xl">Partners who stay close to the work</h2>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {leadership.map((leader) => (
              <article
                key={leader.name}
                className="relative overflow-hidden rounded-3xl border border-white/10 bg-slate-900/55 p-6 text-left"
              >
                <div className="flex h-32 items-center justify-center rounded-2xl border border-dashed border-white/20 bg-slate-950/60 text-xs uppercase tracking-[0.4em] text-slate-500">
                  Team portrait placeholder
                </div>
                <div className="mt-5 space-y-2">
                  <h3 className="text-lg font-semibold text-white">{leader.name}</h3>
                  <p className="text-sm font-medium text-cyan-200/85">{leader.role}</p>
                  <p className="text-sm leading-relaxed text-slate-300/85">{leader.focus}</p>
                </div>
              </article>
            ))}
          </div>
