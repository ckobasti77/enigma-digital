...
export function ScrollStoryHero() {
  ...
  const navigateToIndex = useCallback(
    (targetIndex: number) => {
      ...
      // Lock input to prevent trackpad inertial scrolling glitches
      inputLockUntilRef.current = performance.now() + duration + 350;
  ...
  const shouldTrapInput = useCallback((direction: number) => {
    ...
    const isSectionInViewport = rect.top <= 10 && rect.bottom >= window.innerHeight - 10;
    ...
    if (direction < 0 && currentIndex <= 0 && rect.top >= -5) {
      return false;
    }
  ...
  useEffect(() => {
    ...
    const handleTouchEnd = (event: TouchEvent) => {
      if (touchStartYRef.current === null || !introCompleteRef.current || isAnimatingRef.current) {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (!introCompleteRef.current || isAnimatingRef.current || performance.now() < inputLockUntilRef.current) {
        return;
      }
        ...
        <div className="relative z-10 mx-auto flex h-full w-full max-w-[1690px] flex-col px-4 sm:px-7 lg:px-10 xl:px-12">
          ...
          >
            <button
              ...
              disabled={!isIntroComplete || activeIndex >= LAST_STORY_INDEX}
